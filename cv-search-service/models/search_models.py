...
class SearchResult(BaseModel):
    ...
    matches: List[SearchMatch] = Field(
        ..., description="List of matching text segments"
    )
    summary: Optional[str] = Field(None, description="CV summary")
    skills: Optional[List[str]] = Field(None, description="List of skills")
    experiences: Optional[List[JobExperience]] = Field(
        None, description="List of experiences"
    )
    education: Optional[List[Dict[str, Any]]] = Field(
        None, description="List of education"
    )
    years_experience: Optional[float] = Field(None, description="Years of experience")
    location: Optional[Dict[str, Optional[str]]] = Field(
...
class GraphSearchQuery(BaseModel):
    ...
    technologies: Optional[List[str]] = Field(
        None, description="Technologies to search for"
    )
...
class HybridSearchQuery(BaseModel):
    ...
    technologies: Optional[List[str]] = Field(
        None, description="Technologies to filter by"
    )
...
class FilterOptions(BaseModel):
    ...
    skills: List[FilterOption] = Field(
        default_factory=list, description="Available skills"
    )
    technologies: List[FilterOption] = Field(
        default_factory=list, description="Available technologies"
    )
    roles: List[FilterOption] = Field(
        default_factory=list, description="Available roles"
    )
    companies: List[FilterOption] = Field(
        default_factory=list, description="Available companies"
    )
    locations: List[FilterOption] = Field(
        default_factory=list, description="Available locations"
    )
