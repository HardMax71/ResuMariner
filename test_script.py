...
class CVProcessingTest:
    def __init__(
        self, intake_url="http://intake.localhost", search_url="http://search.localhost"
    ):
    ...
    def process_file(
        self, file_path, parallel=False, generate_review=True, store_in_db=True
    ):
        ...
        try:
            ...
            for i in tqdm(range(max_retries)):
                response = requests.get(
                    f"{self.intake_url}/api/v1/status/{job_id}", timeout=10
                )
                ...
                if status.get("status") == "completed":
                    ...
                    if "result_url" in status and not status.get("data"):
                        ...
                        if result_response.status_code != 200:
                            print(
                                f"Failed to get result: {result_response.status_code} - {result_response.text}"
                            )
                            sys.exit(1)
    ...
    def extract_search_parameters(self):
        ...
        # Extract role
        if (
            self.cv_data.get("professional_profile", {})
            .get("preferences", {})
            .get("role")
        ):
        ...
        # Extract location
        location = (
            self.cv_data.get("personal_info", {})
            .get("demographics", {})
    ...
    def search_semantic(self, query, limit=5, min_score=0.0, filters=None):
        ...
        try:
            ...
            response = requests.post(
                f"{self.search_url}/search/semantic", json=search_payload, timeout=30
            )
            if response.status_code != 200:
                print(
    ...
    def search_structured(
        ...
        try:
            ...
            print(f"Payload: {json.dumps(search_payload, indent=2)}")
            response = requests.post(
                f"{self.search_url}/search/structured", json=search_payload, timeout=30
            )
            if response.status_code != 200:
                print(
    ...
    def search_hybrid(
        ...
        try:
            ...
            print(f"Payload: {json.dumps(search_payload, indent=2)}")
            response = requests.post(
                f"{self.search_url}/search/hybrid", json=search_payload, timeout=30
            )
            if response.status_code != 200:
    ...
    def print_search_results(
        self, results, search_type="", search_params="", include_matches=False
    ):
...
def main():
    ...
    parser.add_argument(
        "--intake-url", default="http://intake.localhost", help="URL for intake service"
    )
    parser.add_argument(
        "--search-url", default="http://search.localhost", help="URL for search service"
    )
    parser.add_argument(
        "--no-review", action="store_true", help="Skip generating review"
    )
    parser.add_argument(
        "--no-store", action="store_true", help="Skip storing in database"
    )
    ...
    client = CVProcessingTest(intake_url=args.intake_url, search_url=args.search_url)
