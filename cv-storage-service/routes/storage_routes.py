...
async def store_cv(request: StoreRequest):
    ...
    try:
        ...
        if is_update:
            logger.info(
                f"Updated existing CV for {email} in graph database with ID: {graph_id}"
            )
        else:
        ...
        raise HTTPException(
            status_code=500, detail=f"Storage operation failed: {str(e)}"
        )
...
async def store_vectors(request: VectorStoreRequest):
    ...
    try:
        ...
        return VectorStoreResponse(status="success", vector_count=len(vector_ids))
...
async def delete_cv(cv_id: str):
    ...
    try:
        ...
        logger.info(
            f"Deleted {vector_count} vectors for CV {cv_id} from vector database"
        )
